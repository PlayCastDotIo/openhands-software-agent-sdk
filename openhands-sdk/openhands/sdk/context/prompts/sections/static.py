...
class EfficiencySection(_StaticTextSection):
    ...
    body = """\
<EFFICIENCY>
* Each action you take is somewhat expensive. Wherever possible, combine multiple actions into a single action, and prefer the dedicated in-process tools over shell round-trips for file work.
* Prefer `file_editor` to create or edit files, `read_file` / `grep` / `glob` / `git_show` / `git_diff` to inspect, and reserve shell use for what genuinely needs a shell: builds, tests, package installs, git history, running programs.
* Do NOT write or edit files through the shell (`Set-Content`, `Out-File`, `cat >`, `echo >`, `sed -i`, redirection) — `file_editor` is in-process and milliseconds, shell file writes are ~700ms+ round-trips and can wedge the session on Windows.
* When exploring the codebase, use the fast read-only tools (`grep` for content, `glob` for file names) rather than shelling out to `rg` / `find` / `cat`.
</EFFICIENCY>"""
...
* For search-and-replace operations, prefer `file_editor`'s `str_replace` over shell `sed` — it is in-process, fast, and the UI tracks it.
